...
<?php
	 ...
	 $codigo_administrador = $_POST["c_codigo_administrador"];
...
<?php
           ...
           else
           {
				...
				$consulta_administradores = "SELECT * FROM $table_administradores WHERE codigo = '$codigo_administrador'";
                $resultado_consulta_administradores = mysqli_query($link, $consulta_administradores);
                $vetor_administradores = mysqli_fetch_array($resultado_consulta_administradores);
                ...
                $consulta_clientes = mysqli_query($link, "SELECT * FROM $table_administradores WHERE cpf = '$cpf' AND codigo != '$codigo_administrador'");
...
<?php
                         ...
                         $inserir_logins = mysqli_query($link, "UPDATE $table_logins SET nickname = '$nickname' , email = '$email' , senha = '$senha' , status = 1 WHERE codigo = '$codigo_login'");
...
<?php
                         ...
                         else
                         {
                              ...
                              $inserir_administradores = mysqli_query($link, "UPDATE $table_administradores SET nome='$nome',sobrenome='$sobrenome',nascimento='$nasc',sexo='$sexo',telefone='$telefone',
                              celular='$celular',cpf='$cpf',codigo_login = '$vetor[0]',rua='$rua',numero='$numero',bairro='$bairro',cep='$cep',cidade='$cidade',estado='$estado' WHERE codigo = '$codigo_administrador'");
                              
                              //Verificando se o cliente foi cadastrado com sucesso
                              
                              if($inserir_administradores == 0)
                              {
?>
                                   //Houve erro no cadastro
                                   <script language="JavaScript">
                                        alert("Erro ao atualizar o ADMINISTRADOR. Entre em contato com o desenvolvedor do sistema");
                                        history.go(-1);
                                   </script>
<?php
                              }

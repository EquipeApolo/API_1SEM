...
sul = tudo2[tudo2['Região']  == 'SUL']

agrupa_id_unico = sul.groupby(by='id_opr_cad_pos').size()
soma_sul = sul.groupby(['id_opr_cad_pos']).sum()
soma_sul['Média'] = soma_sul['Indicador de Pontualidade'] / agrupa_id_unico
media_sul = soma_sul['Média'].sum()/len(soma_sul) 
dropar = sul.drop_duplicates('doc_cli') 


indicador_pontualidade(media_sul)

agrupa_id = sul.groupby(by='id_opr_cad_pos').mean()
...
data = [trace]
fig_sul_cli = go.Figure(data = data)
fig_sul_cli.show()

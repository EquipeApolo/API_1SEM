...
norte = tudo2[tudo2['Região'] == 'NORTE']

agrupa_id_unico = norte.groupby(by='id_opr_cad_pos').size()
soma_norte = norte.groupby(['id_opr_cad_pos']).sum()
soma_norte['Média'] = soma_norte['Indicador de Pontualidade'] / agrupa_id_unico
media_norte = soma_norte['Média'].sum() / len(soma_norte)
dropar = norte.drop_duplicates('doc_cli')

indicador_pontualidade(media_norte)

agrupa_id = norte.groupby(by='id_opr_cad_pos').mean()
...
data = [trace]
fig_norte_cli = go.Figure(data=data)
fig_norte_cli.show()

...
sudeste = tudo2[tudo2['Região'] == 'SUDESTE']

agrupa_id_unico = sudeste.groupby(by='id_opr_cad_pos').size()
soma_sudeste = sudeste.groupby(['id_opr_cad_pos']).sum()
soma_sudeste['Média'] = soma_sudeste['Indicador de Pontualidade'] / agrupa_id_unico
media_sudeste = soma_sudeste['Média'].sum() / len(soma_sudeste)
dropar = sudeste.drop_duplicates('doc_cli')

indicador_pontualidade(media_sudeste)

agrupa_id = sudeste.groupby(by='id_opr_cad_pos').mean()
...
data = [trace]
fig_sudeste_cli = go.Figure(data=data)
fig_sudeste_cli.show()

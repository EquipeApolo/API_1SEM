...
creditos = resultado2[['id','doc_cli','idc_sexo','ano_dat_nascimento', 'des_estado','id_opr_cad_pos','sdo_ddr_tfm','cod_mdl']]
...
tudo2 = tudo[['doc_cli','idc_sexo','nova_coluna','Idade','des_estado','id_opr_cad_pos','sdo_ddr_tfm','vlr_pgt_tfm','vlr_tot_fat_tfm','vlr_min_fat_tfm']]
...
nordeste = tudo2[tudo2['Região']  == 'NORDESTE']


agrupa_id_unico = nordeste.groupby(by='id_opr_cad_pos').size()
soma_nordeste = nordeste.groupby(['id_opr_cad_pos']).sum()
soma_nordeste['Média'] = soma_nordeste['Indicador de Pontualidade'] / agrupa_id_unico
media_nordeste = soma_nordeste['Média'].sum()/len(soma_nordeste) 
drop_doc = nordeste.drop_duplicates('doc_cli') 


#indicador_pontualidade(media_nordeste)

agrupa_id = nordeste.groupby(by='id_opr_cad_pos').mean()
concatena = pd.merge(drop_doc,agrupa_id, on=['id_opr_cad_pos'])
...
# Change the bar mode
fig2.update_layout(barmode='group', title = 'Quantidade de bons pagadores por faixa de idade')
#fig2.show()

Bons_pagadores = pd.DataFrame([val_total_vlr_fatura, val_total_vlr_fatura_media, int(sexo[0]), int(sexo[1]), idade_media ], 
